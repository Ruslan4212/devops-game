...
def("docker", (a, w) => {
  ...
  if (sub === "build") {
    ...
    return O(steps.map((s, ix) => "Step " + (ix + 1) + "/" + steps.length + " : " + s.trim() + "\n ---> " + sha()).join("\n") +
      "\nSuccessfully built " + sha() + "\nSuccessfully tagged " + tag);
  }

  if (sub === "images")
    return O("REPOSITORY:TAG       LAYERS\n" +
  ...
  if (sub === "run") {
    ...
    const image = rest.filter((x) => !x.startsWith("-")).filter((x) => !skip.includes(x)).pop();
    if (!image) return E("docker run: укажи образ");
    if (!d.images.find((i) => i.tag === image))
      return E("Unable to find image '" + image + "' locally — сначала собери его (docker build -t " + image + " .)");
    const name = nameI >= 0 ? rest[nameI + 1] : "cnt" + (d.containers.length + 1);
    let hostPort: number | null = null, cPort: number | null = null;
    if (p >= 0 && rest[p + 1]) {
      const [h, c] = rest[p + 1].split(":");
      hostPort = Number(h); cPort = Number(c);
    }
    d.containers.push({ name, image, state: "running", hostPort, cPort, logs: ["Starting server on :" + (cPort || 80), "listening"] });
    return O(sha());
  }

  if (sub === "ps") {
    const list = rest.includes("-a") ? d.containers : d.containers.filter((c) => c.state === "running");
    return O("NAME        IMAGE            STATUS    PORTS\n" +
      (list.length
        ? list.map((c) => c.name.padEnd(12) + c.image.padEnd(17) + c.state.padEnd(10) + (c.hostPort ? c.hostPort + "->" + c.cPort : "")).join("\n")
        : "(нет контейнеров)"));
  }
  ...
  if (sub === "tag") { d.images.push({ tag: rest[1], layers: 5 }); return O(); }
  if (sub === "push") {
    w.registry.push(rest[0]);
    return O("The push refers to repository [" + rest[0] + "]\nlatest: digest: sha256:" + sha() + " — образ в реестре");
  }
