...
const ownerCanExec = (path: string) => (w: World): boolean => {
...
function seedNginxActive(w: World): void {
  w.services.nginx = {
    desc: "A high performance web server", state: "active", enabled: true, needsPort: 80,
...
export const act02: Lesson[] = [
  {
    id: "2.1", act: 2, title: "Читаем права по буквам", xp: 15,
    ...
    steps: [
      ...
      { kind: "do", text:
        "Задача: выведи подробный список файлов и найди в нём secrets.txt",
        check: ran(/^ls\s+-l/),
        answer: "ls -l",
        hint: "Команда  ls  с флагом  -l" },
      { kind: "quiz", text: "В правах  -rw-r--r--  что может делать владелец файла?",
        options: ["Читать и менять файл", "Только читать", "Читать, менять и запускать"],
        answer: 0, explain: "Группа владельца — это  rw- . Есть r (читать) и w (менять). На третьем месте прочерк, значит запускать нельзя." },
    ],
  },
  {
    id: "2.2", act: 2, title: "Права цифрами: chmod", xp: 20,
    ...
    steps: [
      ...
      { kind: "watch", run: "ls -l secrets.txt", note:
      ...
      { kind: "watch", run: "chmod 600 secrets.txt", note: "Тишина. chmod, как и cd, молчит, когда всё получилось." },
      { kind: "watch", run: "ls -l secrets.txt", note:
        "Стало  -rw-------\n\n" +
      ...
      { kind: "do", text:
        "Задача: сделай скрипт  deploy.sh  запускаемым.",
        check: ownerCanExec("/home/devops/deploy.sh"),
        answer: "chmod +x deploy.sh",
        hint: "Команда  chmod , потом  +x , потом имя файла  deploy.sh" },
      { kind: "quiz", text: "Что даёт группе команда  chmod 640 файл ?",
        options: ["r-- — только чтение", "rw- — чтение и запись", "Ничего"],
        answer: 0, explain: "6 = rw- владельцу, 4 = r-- группе, 0 = ничего остальным." },
    ],
  },
  {
    id: "2.3", act: 2, title: "Что сейчас запущено: ps", xp: 15,
    ...
    steps: [
      ...
      { kind: "quiz", text: "PID — это...",
        options: ["Номер процесса, по нему процессом управляют", "Имя пользователя", "Сколько памяти занято"],
        answer: 0, explain: "PID = Process ID. Уникальный номер работающей программы." },
    ],
  },
  {
    id: "2.4", act: 2, title: "Кто ест процессор: top", xp: 15,
    ...
    steps: [
      ...
      { kind: "do", text:
        "Задача: посмотри список по нагрузке ещё раз, сам.",
        check: (w) => lastCmd(w) === "top",
        answer: "top",
        hint: "Всего три буквы:  top" },
      { kind: "quiz", text: "Чем  top  удобнее  ps , когда сервер тормозит?",
        options: ["Сортирует по нагрузке — виновник сразу сверху", "Показывает меньше процессов", "Работает быстрее"],
        answer: 0, explain: "top ставит самый прожорливый процесс первым, искать глазами не нужно." },
    ],
  },
  {
    id: "2.5", act: 2, title: "Остановить процесс: kill", xp: 20,
    ...
    steps: [
      ...
      { kind: "do", text:
        "Задача: заверши процесс-обжору 3312 с правами администратора.",
        check: (w) => !w.procs.find((p) => p.pid === 3312),
        answer: "sudo kill -9 3312",
        hint: "То же самое, но впереди  sudo , и добавь  -9 :  sudo kill -9 3312" },
      { kind: "quiz", text: "Зачем нужен  sudo  перед  kill  чужого процесса?",
        options: ["Это защита: трогать чужое можно только с правами администратора", "Чтобы команда работала быстрее", "sudo вообще не нужен"],
        answer: 0, explain: "Без sudo система не даст вмешаться в процессы другого пользователя." },
    ],
  },
  {
    id: "2.6", act: 2, title: "Сервисы: кто держит программы живыми", xp: 20,
    ...
    steps: [
      ...
      { kind: "watch", run: "systemctl status nginx", note:
        "Две важные строки:\n\n" +
      ...
      { kind: "type", text: "Спроси состояние сам. Набери:  systemctl status nginx", cmd: "systemctl status nginx" },
      { kind: "quiz", text: "systemd — это...",
        options: ["Системный надзиратель, который держит сервисы запущенными", "Текстовый редактор", "Команда удаления файлов"],
        answer: 0, explain: "systemd запускает сервисы, следит за ними и поднимает их при необходимости." },
    ],
  },
  {
    id: "2.7", act: 2, title: "Поднять сервис и включить автозапуск", xp: 20,
    intro: "start — запустить сейчас, enable — запускать после перезагрузки.",
    setup: (w) => {
      w.services.nginx = { desc: "A high performance web server", state: "inactive", enabled: false, needsPort: 80, journal: [] };
    },
    steps: [
      { kind: "say", text:
      ...
      { kind: "type", text: "Запусти сервис сам. Набери:  sudo systemctl start nginx", cmd: "sudo systemctl start nginx" },
      { kind: "do", text:
        "Задача: включи автозапуск nginx, чтобы он поднимался после перезагрузки сервера.",
        check: (w) => !!w.services.nginx?.enabled,
        answer: "sudo systemctl enable nginx",
        hint: "sudo systemctl enable  и имя сервиса  nginx" },
      { kind: "quiz", text: "Что делает  systemctl enable ?",
        options: ["Сервис будет сам запускаться после перезагрузки сервера", "Запускает сервис прямо сейчас", "Удаляет сервис"],
        answer: 0, explain: "enable — про автозапуск при загрузке. Чтобы запустить сейчас, нужен start." },
    ],
  },
  {
    id: "2.8", act: 2, title: "Читать журнал сервиса", xp: 15,
    intro: "status говорит ЧТО сломалось, журнал говорит ПОЧЕМУ.",
    setup: (w) => {
      w.services.nginx = {
        desc: "A high performance web server", state: "failed", enabled: true, needsPort: 80,
    ...
    steps: [
      ...
      { kind: "watch", run: "systemctl status nginx", note:
        "  Active: failed  — сервис упал. Что именно не так, из этой строки не понять. Идём в журнал." },
      { kind: "watch", run: "journalctl -u nginx", note:
      ...
      { kind: "quiz", text: "Что показывает  journalctl -u nginx ?",
        options: ["Записи журнала именно по сервису nginx", "Все файлы в текущей папке", "Список запущенных процессов"],
        answer: 0, explain: "-u = unit, то есть отфильтровать журнал по одному сервису." },
    ],
  },
  {
    id: "2.9", act: 2, title: "Инцидент: nginx не стартует ⚡", xp: 45,
    ...
    steps: [
      ...
      { kind: "do", text:
        "Шаг 3. Посмотри, какие программы слушают порты.",
        check: ran(/^ss\b/),
        answer: "ss -ltn",
  ...
  {
    id: "2.10", act: 2, title: "Restart и шторм перезапусков ⚡", xp: 30,
    intro: "Авто-перезапуск без ограничителя способен положить весь сервер.",
    setup: (w) => {
      w.services.nginx = {
        desc: "A high performance web server", state: "auto-restart", enabled: true,
        restart: "always", badConfig: true, restartCount: 12400, journalLines: 15000,
    ...
    steps: [
      ...
      { kind: "do", text:
        "Задача: покажи unit-файл (файл настроек) сервиса nginx.",
      ...
      { kind: "do", text:
        "Задача: проверь, насколько заполнен диск.",
        check: ran(/^df\b/),
        answer: "df -h",
      ...
      { kind: "do", text:
        ...
        answer: "[Service]\nRestart=on-failure\nRestartSec=5\nStartLimitIntervalSec=60\nStartLimitBurst=5",
        editFile: OVERRIDE,
        hint: "Открой  edit /etc/systemd/system/nginx.service.d/override.conf  и впиши ровно это:\n" +
          "[Service]\nRestart=on-failure\nRestartSec=5\nStartLimitIntervalSec=60\nStartLimitBurst=5" },
      { kind: "say", text:
      ...
      { kind: "quiz", text: "Что делает строка  StartLimitBurst=5 ?",
        options: [
        ...
        answer: 0,
        explain: "Это ограничитель числа попыток. Без него Restart=always при сломанном конфиге устраивает шторм. Пауза между попытками — это другая строка, RestartSec." },
      { kind: "say", text:
