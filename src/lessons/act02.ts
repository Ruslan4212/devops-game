    ...
    steps: [
      {
        kind: "say",
        text:
          "У обычного раздела (как sdb1 из прошлого урока) есть проблема: если данные\n" +
          "перерастут диск, раздел просто некуда расширить. LVM (Logical Volume Manager,\n" +
          "«менеджер логических томов») решает это через три слоя:\n\n" +
          "  физический том (PV)  — реальный диск, отданный под LVM\n" +
          "  группа томов (VG)    — общий пул из одного или нескольких PV\n" +
          "  логический том (LV)  — кусок этого пула нужного размера, который и монтируют\n\n" +
          "Расширить LV можно, просто добавив ещё один диск в его VG — без пересоздания всего.",
      },
      {
        kind: "watch",
