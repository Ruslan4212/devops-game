...
export const act09: Mission[] = [
  {
    ...
    objs: [
      {
        t: "Опиши Deployment в deploy.yaml",
        d: "edit deploy.yaml",
        ok: has("/home/devops/k8s/deploy.yaml", /kind:\s*Deployment/),
      },
      {
        t: "Задай количество реплик",
        d: "строка replicas: 3",
        ok: has("/home/devops/k8s/deploy.yaml", /replicas:\s*[2-9]/),
      },
      {
        t: "Примени манифест в кластер",
        d: "kubectl apply -f deploy.yaml",
        ok: (w) => !!w.k8s && w.k8s.deploys.length > 0,
      },
      {
        t: "Проверь, что поды запустились",
        d: "kubectl get pods",
        ok: (w) => !!w.k8s && w.k8s.pods.length > 0 && w.log.some((l) => /kubectl\s+get\s+pod/.test(l.cmd)),
      },
      {
        t: "Смасштабируй до 5 реплик",
        d: "kubectl scale deployment/api --replicas=5",
        ok: (w) => !!w.k8s && w.k8s.deploys.some((d) => d.replicas >= 5),
      },
    ],
    solution: [
  ...
  },
  {
    ...
    objs: [
      ...
      {
        t: "Посмотри события кластера по поду",
        d: "kubectl describe pod ИМЯ",
        ok: ran(/^kubectl\s+describe/),
      },
      {
        t: "Добавь недостающую переменную DB_URL в манифест",
        d: "edit deploy.yaml",
        ok: has("/home/devops/k8s/deploy.yaml", /DB_URL/),
      },
      {
        t: "Примени исправленный манифест",
        d: "kubectl apply -f deploy.yaml",
        ok: (w) => !!w.k8s && w.k8s.deploys.every((d) => !d.crash),
      },
      {
        t: "Убедись, что все поды в статусе Running",
        d: "kubectl get pods",
        ok: (w) => !!w.k8s && w.k8s.pods.length > 0 && w.k8s.pods.every((p) => p.status === "Running"),
      },
    ],
    solution: [
      "kubectl get pods",
