...
export const act04: Mission[] = [
  ...
  },
  {
    ...
    objs: [
      ...
      {
        t: "Попробуй достучаться — увидишь таймаут",
        d: "curl https://api.shop.local",
        ok: ranAny(/^curl\b.*api\.shop\.local/),
      },
      ...
      {
        t: "Убедись, что сервис теперь отвечает",
        d: "curl https://api.shop.local",
        ok: (w) => w.log.some((l) => /curl.*api\.shop\.local/.test(l.cmd) && l.code === 0),
      },
    ],
    solution: [
      "dig api.shop.local",
      "curl https://api.shop.local",
      "sudo ufw status",
      "sudo ufw allow 443",
      "curl https://api.shop.local",
    ],
