...
export const act08: Mission[] = [
  {
    ...
    objs: [
      {
        t: "Опиши ресурс в main.tf",
        d: "edit main.tf",
        ok: has("/home/devops/infra/main.tf", /resource\s+"[^"]+"\s+"[^"]+"/),
      },
    ...
    solution: [
      {
        file: "main.tf",
        content: 'resource "local_file" "config" {\n  filename = "app.conf"\n  content  = "env=prod"\n}\n',
      },
      "terraform init",
      "terraform plan",
      "terraform apply",
  ...
  },
  {
    id: "8.2",
    act: 8,
    title: "Инцидент: план сносит прод",
    xp: 75,
    incident: true,
    ...
    objs: [
      {
        t: "Посмотри план и заметь разрушающее изменение",
        d: "terraform plan",
        ok: ran(/^terraform\s+plan/),
      },
      {
        t: "Верни в main.tf потерянный ресурс database",
        d: "edit main.tf",
        ok: has("/home/devops/infra/main.tf", /resource\s+"local_file"\s+"database"/),
      },
      {
        t: "Убедись, что новый план ничего не удаляет",
        d: "terraform plan",
        ok: (w) => !!w.tf.plan && w.tf.plan.del.length === 0,
      },
      {
        t: "Примени безопасный план",
        d: "terraform apply",
        ok: (w) => w.tf.applied.includes("local_file.database"),
      },
    ],
    solution: [
      "terraform plan",
