...
export const act06: Mission[] = [
  {
    ...
    objs: [
      {
        t: "Напиши Dockerfile с базовым образом (FROM)",
        d: "edit Dockerfile",
        ok: has("/home/devops/app/Dockerfile", /^\s*FROM\s+\S+/im),
      },
      {
        t: "Укажи, что запускать в контейнере (CMD)",
        d: "строка CMD в Dockerfile",
        ok: has("/home/devops/app/Dockerfile", /^\s*(CMD|ENTRYPOINT)\s+/im),
      },
      {
        t: "Собери образ с тегом shop:1.0",
        d: "docker build -t shop:1.0 .",
        ok: (w) => w.docker.images.some((i) => i.tag === "shop:1.0"),
      },
      { t: "Проверь, что образ появился", d: "docker images", ok: ran(/^docker\s+images/) },
    ],
    solution: [{ file: "Dockerfile", content: DOCKERFILE }, "docker build -t shop:1.0 .", "docker images"],
  },
  {
    id: "6.2",
    act: 6,
    title: "Запустить и заглянуть внутрь",
    xp: 60,
    ...
    objs: [
      {
        t: "Запусти контейнер в фоне с портом 8080→80 и именем web",
        d: "docker run -d -p 8080:80 --name web shop:1.0",
        ok: (w) => w.docker.containers.some((c) => c.hostPort === 8080),
      },
      { t: "Убедись, что контейнер работает", d: "docker ps", ok: ran(/^docker\s+ps/) },
      {
        t: "Проверь, что сервис отвечает по HTTP",
        d: "curl localhost:8080",
        ok: (w) => w.log.some((l) => /curl.*8080/.test(l.cmd) && l.code === 0),
      },
