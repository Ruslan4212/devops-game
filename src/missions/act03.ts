...
export const act03: Mission[] = [
  {
    ...
    objs: [
      {
        t: "Создай backup.sh и добавь шебанг в первую строку",
        d: "edit backup.sh",
        ok: has("/home/devops/backup.sh", /^#!\/bin\/(ba)?sh/),
      },
      {
        t: "Добавь в скрипт команду echo",
        d: 'внутри редактора: echo "бэкап начат"',
        ok: has("/home/devops/backup.sh", /^\s*echo\s+/m),
      },
  ...
  },
  {
    id: "3.2",
    act: 3,
    title: "Переменные и код возврата",
    xp: 50,
    ...
    objs: [
      {
        t: "Задай переменную окружения APP_ENV",
        d: "export APP_ENV=prod",
        ok: (w) => w.env.APP_ENV === "prod",
      },
      { t: "Выведи её значение", d: "echo $APP_ENV", ok: ran(/^echo\s+\$APP_ENV/) },
      {
        t: "Выполни команду, которая завершится ошибкой",
        d: "ls /нет-такого",
        ok: (w) => w.log.some((l) => l.code !== 0),
      },
  ...
  },
  {
    id: "3.3",
    act: 3,
    title: "Скрипт, который проверяет сервис",
    xp: 60,
    ...
    objs: [
      {
        t: "Напиши healthcheck.sh с шебангом",
        d: "edit healthcheck.sh",
        ok: has("/home/devops/healthcheck.sh", /^#!\/bin\/(ba)?sh/),
      },
      {
        t: "Внутри скрипта должен быть поиск ошибок в логе",
        d: "строка: grep -i error /var/log/app.log",
        ok: has("/home/devops/healthcheck.sh", /grep[^\n]*error[^\n]*app\.log/i),
      },
      {
        t: "Сделай исполняемым и запусти",
        d: "chmod +x healthcheck.sh, затем ./healthcheck.sh",
        ok: (w) => (w.scriptRan || 0) > 0,
      },
    ],
    solution: [
      {
