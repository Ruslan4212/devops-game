...
const seedCiRepo = (w: World): void => {
  ...
  w.git = {
    ...
    remote: "git@github.com:shop/api.git", pushed: 0,
  };
};

export const act07: Mission[] = [
  {
    id: "7.1", act: 7, title: "Собрать пайплайн", xp: 70,
    ...
    objs: [
      { t: "Создай файл пайплайна с триггером на push", d: "edit .github/workflows/ci.yml", ok: has("/home/devops/app/.github/workflows/ci.yml", /on:\s*push/) },
      { t: "Добавь шаг запуска тестов", d: "строка: - run: npm test", ok: has("/home/devops/app/.github/workflows/ci.yml", /npm test/) },
      { t: "Закоммить и запушь — это запустит пайплайн", d: "git add . затем git commit -m \"ci\" и git push", ok: (w) => w.ci.runs.length > 0 },
  ...
  {
    id: "7.2", act: 7, title: "Инцидент: красный билд", xp: 70, incident: true,
    ...
    objs: [
      ...
      { t: "Исправь ошибку в коде", d: "edit src/sum.js — верни a + b", ok: has("/home/devops/app/src/sum.js", /return\s+a\s*\+\s*b/) },
      { t: "Закоммить и запушь исправление", d: "git add . затем git commit -m \"fix sum\" и git push", ok: (w) => w.ci.runs.length > 1 },
      { t: "Убедись, что пайплайн зелёный", d: "ci status", ok: (w) => w.ci.runs.length > 0 && w.ci.runs[w.ci.runs.length - 1].ok },
    ],
    solution: [
      "ci status",
      "ci logs",
