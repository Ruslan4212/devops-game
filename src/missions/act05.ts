...
export const act05: Mission[] = [
  {
    id: "5.1", act: 5, title: "Первый репозиторий", xp: 50,
    ...
    objs: [
      ...
      { t: "Добавь файлы в индекс", d: "git add .", ok: (w) => !!w.git && (w.git.staged.length > 0 || w.git.commits.length > 0) },
      { t: "Сделай коммит с осмысленным сообщением", d: "git commit -m \"первый коммит\"", ok: (w) => !!w.git && w.git.commits.length > 0 },
      { t: "Проверь историю", d: "git log", ok: ran(/^git\s+log/) },
    ],
    solution: ["git init", "git status", "git add .", "git commit -m \"первый коммит\"", "git log"],
  },
  {
    id: "5.2", act: 5, title: "Ветки и слияние", xp: 55,
    ...
    objs: [
      { t: "Создай ветку feature-cart и перейди в неё", d: "git switch -c feature-cart", ok: (w) => w.git?.branch === "feature-cart" },
      { t: "Измени index.js в редакторе", d: "edit index.js", ok: has("/home/devops/app/index.js", /cart|корзин/i) },
      { t: "Закоммить изменение", d: "git add . затем git commit -m \"добавил корзину\"", ok: (w) => !!w.git && w.git.commits.length > 1 },
      { t: "Вернись в main", d: "git switch main", ok: (w) => w.git?.branch === "main" },
      { t: "Влей ветку feature-cart в main", d: "git merge feature-cart", ok: (w) => !!w.git && (w.git.merged || []).includes("feature-cart") },
    ],
    solution: [
